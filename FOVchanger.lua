...
local fov = gui.Slider(visuals_menu, "fov", "Viewmodel FOV", f1, 0, 120);
...
local noshadow_ref = gui.Reference("Visuals", "Other", "Effects")
local lua_noshadows = gui.Checkbox(noshadow_ref, "lua_noshadows", "No Shadows", false)

function noshadows()
    if lua_noshadows:GetValue() then
...
callbacks.Register("Draw", "No Shadows", noshadows);
